(function(){
  ...
  var PROJ = {"k":0.7986355100472928,"minX":-99.61671665089501,"minY":-49.384686592055914,"scale":17.518530276693838,"offX":36.00000000000006,"offY":42.15902019144775};
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    if(burger&&mm){ burger.addEventListener('click', function(){ var o=mm.classList.toggle('open'); burger.classList.toggle('x',o); }); }
    ...
    document.querySelectorAll('.dv[data-count]').forEach(function(d){
      var t=+d.getAttribute('data-count'), suf=d.getAttribute('data-suffix')||'';
      var paint=function(v){ d.innerHTML=v+(suf?'<span class="sfx">'+suf+'</span>':''); };
      if(reduced){ paint(t); return; }
      var cio=new IntersectionObserver(function(es){ es.forEach(function(e){ if(e.isIntersecting){ var s=null; (function st(ts){ if(!s)s=ts; var p=Math.min((ts-s)/1200,1); paint(Math.round(t*(1-Math.pow(1-p,3)))); if(p<1) requestAnimationFrame(st); } })(null); } }); },{threshold:.12});
      cio.observe(d);
    });

    var teaser=q('#mapTeaser'); if(teaser) renderTeaser(teaser);
    if(q('#mapSvg')) initMap();

    var form=q('#propForm'); if(form) initForm(form);
  });

  function renderTeaser(svg){
    ...
    demo.forEach(function(c){ var xy=project(c[0],c[1]); svg.appendChild(svgEl('circle',{cx:xy[0].toFixed(1),cy:xy[1].toFixed(1),r:4.4,'class':'teaser-dot'})); });
  }

  function initMap(){
    var svg=q('#mapSvg'), g;
    for(g=-120; g<=-70; g+=10){ var a=project(g,48.5), b=project(g,25); svg.appendChild(svgEl('line',{x1:a[0].toFixed(1),y1:a[1].toFixed(1),x2:b[0].toFixed(1),y2:b[1].toFixed(1),'class':'grat'})); }
    for(g=25; g<=45; g+=5){ var c=project(-124,g), d=project(-67,g); svg.appendChild(svgEl('line',{x1:c[0].toFixed(1),y1:c[1].toFixed(1),x2:d[0].toFixed(1),y2:d[1].toFixed(1),'class':'grat'})); }
    svg.appendChild(svgEl('path',{d:US_PATH,'class':'us-line'}));

    var dots=[];
    REGIONS.forEach(function(r,ri){ r.pts.forEach(function(c){ var xy=project(c[0],c[1]); var gg=svgEl('g',{transform:'translate('+xy[0].toFixed(1)+','+xy[1].toFixed(1)+')'}); var pulse=svgEl('circle',{r:'0','class':'dot-pulse'}); var dot=svgEl('circle',{r:'3.6','class':'dot off'}); gg.appendChild(pulse); gg.appendChild(dot); svg.appendChild(gg); dots.push({region:ri,dot:dot,pulse:pulse}); }); });

    var list=q('#mktList'), readout=q('#readout'), btns=[];
    REGIONS.forEach(function(r,i){
      var b=document.createElement('button'); b.className='mkt-item';
    ...
    function select(i){
      ...
      dots.forEach(function(o){ var on=o.region===i; o.dot.setAttribute('class','dot '+(on?'on':'off')); o.dot.setAttribute('r',on?'4.8':'3.6'); o.pulse.setAttribute('class','dot-pulse'+(on&&!reduced?' live':'')); });
  ...
  function initForm(form){
    ...
    form.addEventListener('submit', function(e){
      e.preventDefault();
      var ok=true, name=form.name, email=form.email, message=form.message;
      [name,email,message].forEach(function(inp){ var f=inp.closest('.field'); var bad=!inp.value.trim(); if(inp===email){ bad=!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(inp.value.trim()); } setErr(f,bad); ok=ok&&!bad; });
      if(!ok){ var first=form.querySelector('.field.err input,.field.err textarea'); if(first)first.focus(); return; }
      var subject='Inquiry — '+(form.reason.value||'WS Land');
      var body='Name: '+form.name.value+'\nPhone: '+form.phone.value+'\nEmail: '+form.email.value+'\nRegarding: '+form.reason.value+'\n\nMessage:\n'+form.message.value+'\n';
      var mailto='mailto:wslandacquisitions@gmail.com?subject='+encodeURIComponent(subject)+'&body='+encodeURIComponent(body);
      form.style.display='none'; var s=q('#formSuccess'); if(s)s.classList.add('show'); window.location.href=mailto;
    });
  }
})();
